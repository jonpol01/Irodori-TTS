...
enc_s = ort.InferenceSession("/tmp/irodori_encoder.onnx", so, providers=["CPUExecutionProvider"])
dit_s = ort.InferenceSession("/tmp/irodori_dit.onnx", so, providers=["CPUExecutionProvider"])
dec_s = ort.InferenceSession("/tmp/dacvae_decoder.onnx", so, providers=["CPUExecutionProvider"])
...
p = os.path.expanduser("~/Desktop/irodori_onnx.wav")
